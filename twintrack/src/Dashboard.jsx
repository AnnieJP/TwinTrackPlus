...
function BarChart({ proj_op1, proj_op2 }) {
  ...
  const dataMin = Math.min(...allVals) * 0.97;
  const dataMax = Math.max(...allVals) * 1.02;
  const range   = dataMax - dataMin || 1;
  const n       = proj_op1.length;
  const groupW  = iW / n;
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display:"block" }}>
      ...
      {proj_op1.map((p1, i) => {
        ...
        const lbl = p1.date || `Mo ${p1.month}`;
        return (
          <g key={i}>
            <rect x={cx} y={toY(p1.value)} width={bw} height={bH(p1.value)} fill="rgba(100,130,170,0.45)" rx={3} />
            <rect x={ex} y={toY(p2.value)} width={bw} height={bH(p2.value)} fill="rgba(13,148,136,0.75)" rx={3} />
      ...
      <rect x={padL+90}  y={padT-4} width={10} height={10} fill="rgba(13,148,136,0.75)" rx={2} />
      <text x={padL+104} y={padT+5} fill={C.accentSoft} fontSize={9}>Experiment (OP2)</text>
    </svg>
  );
}
  ...
  return (
    <div style={s.root}>
      <div style={s.inner}>
        ...
        {/* Back */}
        <button onClick={onBack} style={{ display:"inline-flex", alignItems:"center", gap:7, padding:"7px 14px", borderRadius:8, border:`1px solid ${C.border}`, background:"transparent", color:C.textMid, fontSize:12.5, cursor:"pointer", fontFamily:"inherit", marginBottom:28 }}>
          ← Back to overview
        </button>
        ...
        {proj1.length > 0 && (
          <div style={{ ...fade, ...s.card, marginBottom:28, transitionDelay:"0.15s" }}>
            <p style={{ ...s.label, marginBottom:16 }}>6-Month Revenue Projection</p>
